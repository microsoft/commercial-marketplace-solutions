...
namespace ManagedWebhook
{
    public static class Webhook
    {
            ...
            ExecutionContext context)
        {
            ...
            if(plan == null)
                return new BadRequestObjectResult("Please pass a plan on the query string or in the request header");
            ...
            using (var armHttpClient = HttpClientFactory.Create())
            {
                ...
                if (applicationResourceId != null)
                {
                    ...
                    log.LogInformation($"Plan name: {plan}");
                    log.LogInformation($"Dimension: {dimension}");
                    log.LogInformation($"Quantity: {quantity}");

                    var response = await Webhook.EmitUsageEvents(config, armHttpClient, dimension,quantity,effectiveStartTime, applicationResourceId, plan).ConfigureAwait(continueOnCapturedContext: false);
